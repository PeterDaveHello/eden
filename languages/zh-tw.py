...
'A practical example can be of a report of lost person. Now if one machine register him to be found on 16th August and another machine registers him to found on 17th August, then e.g. Newer timestamp will replace data entry of your machine with that of foriegn machine because that is newer one.': '一個實用的範例可以是一個報告的遺失人員。 現在如果一台機器註冊該上找到第16 8月及另一部機器登錄該使用者上找第17 8月,例如,然後新的時間戳記會取代資料項目的機器的外部機器的原因是新的。',
...
'Do women and girls have easy access to sanitary materials?': '婦女做和女孩輕松存取衛生資料嗎?',
...
'Text': '文字',
'Thank you for validating your email. Your user account is still pending for approval by the system administator (%s).You will get a notification by email when your account is activated.': '感謝您驗證您的電子郵件。 您的使用者帳戶仍在擱置中的核准的系統管理者 (%s).,您將得到時,以電子郵件通知您的帳戶已被啟用。',
